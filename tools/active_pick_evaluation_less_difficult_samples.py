...
def PreprocessData(file_name):
    ...
    for index, (image_path, boxes_info) in enumerate(data.items()):
        ...
        _diversity = len(_cls_set) # number of prediction classes
        
        if _difficult==0:
            difficult_indicators[image_path] =1  # The sample is an easy sample, then we set to 1 to increase the chance to be selected 
        if _difficult!=0:
            difficult_indicators[image_path] =1/_difficult
...
def special_(args):
    ...
    _difficult_indicators = norm_dict(difficult_indicators) # 1/difficuty_metric
...
if __name__ == '__main__':
    ...
    parser.add_argument("--static_file",type=str,default='temp/coco/static_by_random10.json') #Json file of the inferenced datasets info
    parser.add_argument("--indicator_file",type=str,default='results/coco/15random_maxnorm') #indicator file to be used in picking data
